...
export class ViewSurveyReportComponent  {
  ...
  constructor(private surveyReportService:SurveyorService , private router: Router,private http:HttpClient) { }
  fetchSurveyReport() {

    this.data = this.claimId
    console.log(this.data)
    
    this.surveyReportService.getSurveyReport(this.data).subscribe(data => {
      console.log(data)
  ...
  update(){
    ...
    console.log(this.data);
    this.surveyReportService.updateSurvey(this.claimId,this.data).subscribe(data => {
    //  JSON.parse(result);
    this.surveyReport = data;
      Swal.fire({
        icon: 'success',
      ...
      this.http.put<Claim>(`http://localhost:8081/api/claims/${data.claimId}/${data.totalAmount}/update`,"").subscribe((response: any) => {
